...
export default function ModsPageInner() {
    ...
    <div data-testid="mods-page" className="pt-28">
      <section className="section-pad pt-0">
        <div className="container-oc">
          ...
          {section === "mods" && <ModsSection cat={cat} setCat={setCat} />}
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          <span
            ...
            aria-hidden
          >
            <CategoryIcon name={activeCat?.icon} size={18} />
          </span>
          <h3 className="font-pixel text-3xl text-white">{activeCat?.label}</h3>
            ...
            >
              <AccordionTrigger className="hover:no-underline">
                <div className="flex items-center gap-3 text-left">
                  <span
                    className="font-accent text-[9px] uppercase tracking-[0.2em] px-2 py-1 flex items-center gap-1.5"
                    style={{
                      background: `${activeCat?.color}1a`,
                      color: activeCat?.color,
                    }}
                  >
                    <CategoryIcon name={activeCat?.icon} size={10} />
                    {activeCat?.label}
                  </span>
                  <span className="font-pixel text-2xl text-white">{m.name}</span>
                </div>
              </AccordionTrigger>
              <AccordionContent className="text-white/70">
                <p>{m.description}</p>
                <div
                  className="mt-3 flex gap-3 border-l-2 pl-3"
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between">
              <span
                className="font-accent text-[9px] uppercase tracking-[0.2em] px-2 py-0.5 flex items-center gap-1.5"
                style={{
                  background: `${c?.color || "#22c55e"}1a`,
                  color: c?.color || "#22c55e",
                }}
              >
                {c && <CategoryIcon name={c.icon} size={10} />}
                {c?.label || d.category}
              </span>
